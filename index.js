	...
	function showOverlay(error, onPageLoad) {
	    ...
	    switch (true) {
	        case rateLimitExceeded:
	            content.append($("<h1>").text(`GitHub API Rate Limit Exceeded`), $("<p>").text(`This ${"page" } uses the GitHub API, which has a rate limit. You have exceeded this limit.`));
	            break;
	        case authenticationRequired:
	            content.append($("<h1>").text(`GitHub Authentication Required`), $("<p>").text(`This ${"page" } requires authentication at GitHub. You need to log in to continue.`));
	            break;
	        default:
	            content.append($("<h1>").text(`Unknown ${error instanceof HTTPError ? "HTTP" : ""} Error`), $("<p>").text(`An error occurred while ${"loading this page" }. Unfortunately this app does not know how to handle it.`));
	            break;
	    }
	    ...
	    $("<button>").addClass("action").text(`Reload the page`).on("click", () => location.reload()) , 
	        ...
	        for await (const issue of streamOpenIssues())
	            buffer = buffer.then(() => {
	                ...
	                $("<div>")
	                    ...
	                    .appendTo("#issues")
	                    .append($("<div>").addClass("header").append($("<span>").addClass("title").text(`#${issue.number} ${issue.title}`), $("<span>").append($("<button>")
	                    .addClass("actual")
	                    .text("Start translating")
	                    .on("click", () => window.open(`edit.html?language=${$("#languages").val()}&issue=${issue.number}`, "_blank")), $("<button>")
	                    .addClass("dummy")
	                    .attr("disabled", "true")
	                    .text("Select a language first"), $("<a>")
	                    .attr("href", issue.html_url)
	                    .attr("target", "_blank")
	                    .append("Show issue on GitHub ↗", $("<img>").attr("src", "github-mark.png")))), (strings => strings.length ? $("<details>").append($("<summary>").text("Strings"), strings.map(str => $("<pre>").text(`${str.strId}: ${str.descNew || str.descOld || ""}`))) : $("<div>").addClass("no-strings").text("No strings found"))(extractTranslationStringsFromIssue(issue.body)));
	            });
	        await buffer;
	        $("#loading").remove();
	    })();
