(function (exports) {
	...
	async function commit(token, data, langFile) {
	            ...
	            for await (const issues of streamOpenIssues()) {
	                issues.filter(issue => !issue.pull_request).forEach(issue => {
	                    ...
	                    $("<details>")
	                        ...
	                        .text(`#${issue.number}`), $("<span>").text(` ${issue.title} (`), $("<span>").css("display", "inline-flex").css("gap", "0.5em").append(missingLanguages.map(language => $("<span>").addClass(language).text(language).on("click", () => edit(issue, language)))), $("<span>").text(`)`)), extractTranslationStrings(issue.body).map(str => $("<pre>").text(str)));
	                });
	                languages.forEach(language => {
	...
	async function edit(issue, language) {
	    $("#language-list").hide();
	    $("#issue-list").hide();
	    ...
	    strings.map(str => str.match(`STR_\\d{4}`)[0]).forEach(strId => $("<tr>").append($("<td>").addClass("strId").text(strId), $("<td>").text(getStr(enGB, strId)), $("<td>").append($("<button>").text(">").on("click", function () { $(this).parent().next().text($(this).parent().prev().text()); })), $("<td>").addClass("translated").attr("contenteditable", "true").text(getStr(lang, strId))).appendTo("#translate-strings tbody"));
	    $("#save-translation").off("click").on("click", () => saveTranslation(lang));
	}
	function getStr(data, str) {
	    const regex = new RegExp(`${str.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}\\s*:([^\n\r]*)`);
	    const match = data.match(regex);
	    return match ? match[1] : "";
	}
	function saveTranslation(langFile) {
	    const data = getData();
	    storeData(data);
	    const token = getToken();
	    if (!token)
